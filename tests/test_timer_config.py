...
def test_next_run_at_schaetzt_jetzt_plus_intervall():
    ...
    eigene Service ist dann noch "active (running)", OnUnitActiveSec kennt
    seinen naechsten Termin erst nach Abschluss dieser Aktivierung). Reine
    Zeitrechnung, kein subprocess/systemd mehr involviert."""
    from datetime import datetime, timedelta

    before = datetime.now()
    result = timer_config.next_run_at(5)
    after = datetime.now()

    parsed = datetime.strptime(result, "%Y-%m-%d %H:%M:%S")
    assert before + timedelta(minutes=5) - timedelta(seconds=1) <= parsed <= after + timedelta(minutes=5)
